	...
	if($children == 0){	#this is data item, not a container
		...
		if(defined $opts{r}){
			...
			if($di eq "FMOD"){
				...
				if($interpreted != 3){
					$interpreted .= " -- version not equal 3: the frame may not be read correctly!";
				}
			}elsif($di eq "BPMT"){
				#BPM
				...
				for(my $i = 0; $i < $cuepoints; $i++){
					...
					if($first != 1){
						...
						$interpreted .= ", displ_order: $displ_order, type: $type_t, start: $start, len: $len, repeats: $repeats, hotcue: $hotcue";
...
my ($data, $name, @rest) = $id3v2->get_frame('PRIV', 'intact');
if(! defined $data){
	print "No PRIV frame\n";
	exit 1;
}

# $name contains "Private frame"
# $data contains one PRIV frame - it is a hash with keys Text and _Data
my $found = 1;
if($data->{'Text'} eq 'TRAKTOR4'){
	decode(0, 0, $data->{'_Data'});
	$found = 1;
}else{
	# there can be more PRIV frames
	# @rest is an array of hashrefs
	for my $rest (@rest){
		if($rest->{'Text'} eq 'TRAKTOR4'){
			decode(0, 0, $rest->{'_Data'});
			$found = 1;
			last;
		}
	}
